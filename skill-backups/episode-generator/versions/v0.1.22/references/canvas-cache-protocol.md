...
- v0.1.22 继承 v0.1.21 的上游转译与 v0.1.19 的逐集语义规则，并强制公开根目录双文件白名单、私有数据不外显和规范进度句式。v0.1.21 另要求`upstream-reference.md`来源指纹正确、字段白名单合格、四项转译记录具体通过且未解决问题为“无”。v0.1.19 每集语义触发登记和三层复核记录栏目完整，人物交流证据逐字引用真实台词并合计覆盖全部对白场次，未解决问题为“无”，且三项正文 SHA-256 均等于私有当前定稿的 SHA-256；完整剧本汇总逐字包含所有冻结定稿。v0.1.18 继续按原字段兼容读取。
