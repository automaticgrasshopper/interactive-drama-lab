...
缓存版本使用 `episode-cache-v0.1.20`。

`执行组状态`每行固定为`- G编号｜节点列表｜状态`，状态只能是：`未开始、已装载、候选正文、机械预检通过、场面事实通过、人物交流通过、隔离观众通过、机械冻结、状态已回写`。已回写组必须连续位于最前；最多一个组处于中间状态，其后全部为未开始。最终公开交付时所有组必须为状态已回写。
...
- 复核方式：完整复核/保全确认/PENDING
- 正文 SHA-256：64位十六进制/PENDING
- 动作骨架 SHA-256：64位十六进制/PENDING
- 触发登记覆盖：PASS｜场次｜具体依据
...
```text
- 修改类型：无/表层台词润色/事实台词修改/动作或场次修改/真实结尾修改/拓扑或正式输入修改
- 修改前正文 SHA-256：64位十六进制/不适用
- 修改前动作骨架 SHA-256：64位十六进制/不适用
- 影响范围：当前集/节点列表/不适用
```

新写分集使用`修改类型：无`和场面事实`复核方式：完整复核`。表层台词润色前，把旧当前定稿完整复制到`上一冻结定稿`并写入修改前两个指纹；新正文动作骨架与上一冻结定稿不一致时不得使用保全确认。
...
`当前集初稿`和`当前集定稿`必须包含实际剧本文本，不得写“已完成”“见公开稿”“与公开稿一致”等替代语。`度量记录`固定记录公开可见字符数、动作段数、门槛和判定。复核记录的旧正文指纹在修改后失效；按主`SKILL.md`的矩阵执行完整复核或保全确认后才能冻结。
...
- v0.1.20 除 v0.1.19 要求外，校验执行组状态单向推进、场面事实复核方式、动作骨架指纹和修改记录。保全确认必须具有真实上一冻结定稿，且修改前后动作骨架一致。v0.1.18—v0.1.19 继续按原字段兼容读取。
...
2. 修改前保存`上一冻结定稿`和修改记录，再写入候选；旧复核指纹立即失效，随后按主修改矩阵执行保全确认或完整复核。
...
6. 未受影响节点保持冻结；当前集三份旧正文指纹失效后，按修改矩阵完成重新绑定和必要复核才可 finalize。
