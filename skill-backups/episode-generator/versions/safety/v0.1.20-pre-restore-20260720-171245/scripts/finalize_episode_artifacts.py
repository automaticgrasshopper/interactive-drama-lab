...
def action_skeleton(text: str) -> str:
    kept: list[str] = []
    in_scene = False
    for line in text.strip().splitlines():
        if re.match(r"^【\s*场[^·】]+?\s*·", line):
            in_scene = True
        if in_scene:
            match = re.match(r"^([^：\n]{1,30})：\s*(\S.*)$", line)
...
def action_digest(text: str) -> str:
    return digest(action_skeleton(text))
...
def pending_v20_scene_audit(fields: tuple[str, ...]) -> str:
    lines = [
        "- 状态：PENDING",
        "- 复核方式：PENDING",
        "- 正文 SHA-256：PENDING",
        "- 动作骨架 SHA-256：PENDING",
    ]
    lines.extend(f"- {name}：PENDING｜待当前主 Agent 复核" for name in fields)
    lines.append("- 未解决问题：待复核")
    return "\n".join(lines)
...
def validate_v20_scene_binding(
    cache_text: str, final_text: str, episode_id: str
) -> None:
    block = section(cache_text, "场面事实复核记录")
    method = field(block, "复核方式")
    current_action_digest = action_digest(final_text)
    if method not in ("完整复核", "保全确认"):
        raise SystemExit(f"{episode_id} 缺少有效场面事实复核方式")
    if field(block, "动作骨架 SHA-256") != current_action_digest:
        raise SystemExit(f"{episode_id} 的动作骨架指纹与当前定稿不一致")
    if method == "完整复核":
        return
    previous = section(cache_text, "上一冻结定稿").strip()
    revision = section(cache_text, "修改记录")
    if len(re.sub(r"\s+", "", previous)) < 300:
        raise SystemExit(f"{episode_id} 保全确认缺少上一冻结定稿")
    if field(revision, "修改类型") != "表层台词润色":
        raise SystemExit(f"{episode_id} 只有表层台词润色可使用保全确认")
    if field(revision, "修改前正文 SHA-256") != digest(previous):
        raise SystemExit(f"{episode_id} 修改前正文指纹不匹配")
    previous_action_digest = action_digest(previous)
    if field(revision, "修改前动作骨架 SHA-256") != previous_action_digest:
        raise SystemExit(f"{episode_id} 修改前动作骨架指纹不匹配")
    if previous_action_digest != current_action_digest:
        raise SystemExit(f"{episode_id} 动作骨架已改变，不能使用保全确认")
...
def main() -> int:
    ...
    parser.add_argument(
        "--mode",
        choices=("record-draft", "prepare-dialogue-revision", "finalize"),
        required=True,
    )
    ...
    is_v20 = "episode-cache-v0.1.20" in manifest
    is_v19 = "episode-cache-v0.1.19" in manifest or is_v20
    ...
    if args.mode == "prepare-dialogue-revision" and not is_v20:
        raise SystemExit("prepare-dialogue-revision 仅用于 episode-cache-v0.1.20")
    ...
    for cache_path in targets:
        ...
        if args.mode == "prepare-dialogue-revision":
            previous = section(cache_text, "当前集定稿").strip()
            if len(re.sub(r"\s+", "", previous)) < 300:
                raise SystemExit(f"{episode_id} 没有可供润色的冻结定稿")
            if "冻结状态：已冻结" not in section(cache_text, "校验状态"):
                raise SystemExit(f"{episode_id} 尚未冻结，不能开始局部台词修改")
            cache_text = replace_section(cache_text, "上一冻结定稿", previous)
            revision = (
                "- 修改类型：表层台词润色\n"
                f"- 修改前正文 SHA-256：{digest(previous)}\n"
                f"- 修改前动作骨架 SHA-256：{action_digest(previous)}\n"
                "- 影响范围：当前集"
            )
            cache_text = replace_section(cache_text, "修改记录", revision)
            cache_text = replace_section(
                cache_text,
                "场面事实复核记录",
                pending_v20_scene_audit(SCENE_FACT_FIELDS),
            )
            cache_text = replace_section(
                cache_text,
                "人物交流复核记录",
                pending_audit(character_exchange_fields),
            )
            cache_text = replace_section(
                cache_text,
                "隔离观众复核记录",
            ...
            if is_v20:
                cache_text = replace_section(cache_text, "上一冻结定稿", "PENDING")
                cache_text = replace_section(
                    cache_text,
                    "修改记录",
                    "- 修改类型：无\n- 修改前正文 SHA-256：不适用\n"
            ...
            if is_v18:
                cache_text = replace_section(
                    ...
                    pending_v20_scene_audit(SCENE_FACT_FIELDS)
                    if is_v20
                    else pending_audit(SCENE_FACT_FIELDS),
                )
                cache_text = replace_section(
                    cache_text,
                    "人物交流复核记录",
            ...
            if is_v18:
                ...
                if is_v20:
                    validate_v20_scene_binding(cache_text, candidate_text, episode_id)
            else:
                validate_audit(
                    cache_text,
                    "因果复核记录",
    ...
    if args.mode == "finalize":
        ...
        all_frozen = all(
            "冻结状态：已冻结"
            in section(path.read_text(encoding="utf-8"), "校验状态")
            for path in all_cache
        )
        if args.episodes and not all_frozen:
            return 0
