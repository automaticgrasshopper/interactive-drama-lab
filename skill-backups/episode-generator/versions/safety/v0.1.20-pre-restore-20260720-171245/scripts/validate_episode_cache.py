...
def validate_group_states(manifest: str, require_public: bool, errors: list[str]) -> None:
    block = section(manifest, "执行组状态")
    rows = re.findall(r"^-\s*(G\d+)｜([^｜]+)｜(\S+)\s*$", block, re.MULTILINE)
    if not rows:
        errors.append("manifest.md 缺少有效执行组状态")
        return
    statuses = [status for _, _, status in rows]
    invalid = [status for status in statuses if status not in GROUP_STATES]
    if invalid:
        errors.append(f"manifest.md 含无效执行组状态：{invalid}")
    incomplete_seen = False
    active_count = 0
    for status in statuses:
        if status == "状态已回写" and incomplete_seen:
            errors.append("执行组状态不是连续前缀提交")
        elif status != "状态已回写":
            incomplete_seen = True
            if status != "未开始":
                active_count += 1
    if active_count > 1:
        errors.append("同时只能有一个执行组处于中间状态")
    if require_public and any(status != "状态已回写" for status in statuses):
        errors.append("公开交付前所有执行组必须达到状态已回写")
...
def validate_v20_scene_binding(
    ...
    method = audit_field(block, "复核方式")
    current_action_digest = action_digest(final_text)
    if method not in ("完整复核", "保全确认"):
        errors.append(f"{node_id} 缺少有效场面事实复核方式")
    if audit_field(block, "动作骨架 SHA-256") != current_action_digest:
        errors.append(f"{node_id} 的动作骨架指纹与当前定稿不一致")
    if method != "保全确认":
        return
    previous = section(cache_text, "上一冻结定稿").strip()
    revision = section(cache_text, "修改记录")
    if len(re.sub(r"\s+", "", previous)) < 300:
        errors.append(f"{node_id} 保全确认缺少上一冻结定稿")
        return
    if audit_field(revision, "修改类型") != "表层台词润色":
        errors.append(f"{node_id} 只有表层台词润色可使用保全确认")
    if audit_field(revision, "修改前正文 SHA-256") != digest(previous):
        errors.append(f"{node_id} 修改前正文指纹不匹配")
    previous_action_digest = action_digest(previous)
    if audit_field(revision, "修改前动作骨架 SHA-256") != previous_action_digest:
        errors.append(f"{node_id} 修改前动作骨架指纹不匹配")
    if previous_action_digest != current_action_digest:
        errors.append(f"{node_id} 动作骨架已改变，不能使用保全确认")
...
def validate_v12_episode_audits(
    ...
    for node_id in sorted(nodes):
        ...
        if is_v18:
            ...
            if is_v20:
                validate_v20_scene_binding(cache_text, final_text, node_id, errors)
        else:
            validate_audit_block(
...
def validate(
    ...
    if re.search(r"episode-cache-v0\.1\.(?:8|9|10|11|12|13|14|17|18|19|20)\b", manifest):
        branch_audit = read_text(cache / "branch-audit.md", errors)
        if branch_audit:
            validate_branch_audit(branch_audit, nodes, errors)
    if re.search(r"episode-cache-v0\.1\.(?:9|10|11|12|13|14|17|18|19|20)\b", manifest):
        if is_v18:
            manifest_headings = [
                "场面事实复核状态",
                "人物交流复核状态",
        ...
        if is_v20:
            validate_group_states(manifest, require_public, errors)
    ...
    if require_public:
        ...
        if re.search(r"episode-cache-v0\.1\.(?:10|11|12|13|14|17|18|19|20)\b", manifest):
