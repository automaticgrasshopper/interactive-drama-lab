...
def validate(
    ...
    is_v21 = "episode-cache-v0.1.21" in manifest
    ...
    if re.search(r"episode-cache-v0\.1\.(?:8|9|10|11|12|13|14|17|18|19|21)\b", manifest):
        branch_audit = read_text(cache / "branch-audit.md", errors)
        if branch_audit:
            validate_branch_audit(branch_audit, nodes, errors)
    if re.search(r"episode-cache-v0\.1\.(?:9|10|11|12|13|14|17|18|19|21)\b", manifest):
        if is_v18:
            manifest_headings = [
                "场面事实复核状态",
                "人物交流复核状态",
