...
- v0.1.25 继承 v0.1.24，并以确定性台词编号、台词包指纹、总数和覆盖状态替代人物交流层逐句 PASS 证词；模型仍在原人物交流调用中整场判断，只返回带编号的问题项。v0.1.24 把两个公开成果提升为共享 Canvas 根目录的一级文件。v0.1.23 要求全部阶段具有由 `reference-manifest.json` 和当前文件哈希签发的私有装载凭证。v0.1.22 强制分集公开双文件白名单、私有数据不外显和规范进度句式。旧缓存继续按各自版本规则兼容读取。
