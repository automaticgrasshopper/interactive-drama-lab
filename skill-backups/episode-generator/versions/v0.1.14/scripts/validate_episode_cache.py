...
def validate(
    ...
    is_current_cache = is_v12 or is_v13
    if is_current_cache:
    ...
    if re.search(r"episode-cache-v0\.1\.(?:8|9|10|11|12|13)\b", manifest):
        branch_audit = read_text(cache / "branch-audit.md", errors)
        if branch_audit:
            validate_branch_audit(branch_audit, nodes, errors)
    if re.search(r"episode-cache-v0\.1\.(?:9|10|11|12|13)\b", manifest):
    ...
    if require_public:
        ...
        if re.search(r"episode-cache-v0\.1\.(?:10|11|12|13)\b", manifest):
