...
本 Skill 只在共享 Canvas 根目录直接保留一级文件 `episode-flowchart.svg` 和 `episode-script.md`，不创建项目子目录。Canvas 中其它模块已有文件不受影响；不得创建 `episode-structure.md`、`episodes/`、`episode-delivery.md`、交接文件、缓存目录、审计文件、生产卡、运行日志或 HTML 首页。
...
- 非结局有默认推进或选择；
- 分集缓存文件数量与节点数一致；
- Canvas 最终完整剧本节点数与拓扑一致；本 Skill 没有在根目录创建两个规定文件之外的分集产物，不把其它模块文件判为异常。
- 每集可见字符数和动作段数达到 manifest 中记录的项目门槛；未记录时使用 850—1300 个去空白字符、16—24 个动作段。
- 私有缓存保存真实初稿、真实定稿和度量记录，不接受事后引用公开稿的占位文字。
- v0.1.24 继承 v0.1.23，并把两个公开成果提升为共享 Canvas 根目录的一级文件，同时允许其它模块的一级文件共存。v0.1.23 要求全部阶段具有由 `reference-manifest.json` 和当前文件哈希签发的私有装载凭证。v0.1.22 强制分集公开双文件白名单、私有数据不外显和规范进度句式。v0.1.21 另要求`upstream-reference.md`来源指纹正确、字段白名单合格、四项转译记录具体通过且未解决问题为“无”。v0.1.19 每集语义触发登记和三层复核记录栏目完整，人物交流证据逐字引用真实台词并合计覆盖全部对白场次，未解决问题为“无”，且三项正文 SHA-256 均等于私有当前定稿的 SHA-256；完整剧本汇总逐字包含所有冻结定稿。v0.1.18 继续按原字段兼容读取。
...
运行 `python3 {skill-root}/scripts/render_episode_flowchart.py {canvas-root} --cache-root {cache-root} --title "《游戏标题》分集流程图"`，从私有冻结 `topology.md` 直接生成 Canvas 一级静态 SVG。不得复制 Mermaid 源码充当视觉图，也不得依赖宿主页面运行 Mermaid JavaScript；Mermaid、梗概和选项源只留在私有缓存。

每个私有分集缓存通过校验并冻结后，继续作为该集唯一编辑源，不投射公开逐集文件。

`episode-script.md` 由全部私有逐集缓存的已冻结 `当前集定稿` 按编号组装。组装阶段不重新生成全文，不从汇总稿反向拆集，也不创建任何九字段交付文件。

将真实完整初稿直接写入私有分集的 `当前集初稿`，再使用 `python3 {skill-root}/scripts/finalize_episode_artifacts.py {canvas-root} --cache-root {cache-root} --mode record-draft [--episodes episode-001,...]` 核验初稿并把定稿与三份复核状态重置为 PENDING。主 Agent 把修订候选写入 `当前集定稿`，完成场面事实、人物交流和隔离观众复核后，将三份带当前定稿 SHA-256、真实场次定位和具体证据的记录写入缓存，再使用 `--mode finalize` 冻结定稿并重新组装汇总稿。
