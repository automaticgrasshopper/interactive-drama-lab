...
REFERENCE_PATH = EXECUTION_ROOT / "references" / REFERENCE_NAME
...
def current_reference() -> tuple[str, str, str]:
    ...
    version = str(manifest.get("execution_schema_version") or "")
...
def seal(cache_root: Path, episode_id: str, review_path: Path) -> Path:
    ...
    receipt = {
        ...
        "execution_schema_version": packet["execution_schema_version"],
        "episode_id": episode_id,
...
def verify_project(cache_root: Path) -> list[str]:
    ...
    for episode_id in episode_ids:
        ...
        try:
            ...
            continue
        for key in ("packet_version", "execution_schema_version", "episode_id", "script_sha256", "reference_name", "reference_sha256", "required_checks"):
